# ------------------------------------------------------------------------
# rect11.py
# 基于 rect10.py 的修正标定版本：
# 1. 识别流程、ROI 状态机、形态学、矩形查找与校验逻辑保持和 rect10.py 一致。
# 2. 算法修正：由于激光笔与摄像头不在同一直线上，需要寻找摄像头镜头中心与
#    矩形中心偏上 1.5 cm 的目标点，并计算该目标点对应的实际角度（Yaw/Pitch）。
# 3. 理想标定目标：偏上 1.5 cm 的点其偏航角差 (Yaw) 应正好为 0°，俯仰角差 (Pitch) 
...
def main():
    ...
    try:
        print("--- rect11 启动 (Sensor id=2 + 16:9 LAB 标定目标点 IDE 版) ---")
        camera_init()
        camera_is_init = True
